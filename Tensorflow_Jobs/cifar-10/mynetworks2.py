...
def net_build(image):
    ...
    with tf.variable_scope('conv1') as scope:
        kernel = weight_variable('weights', [5, 5, 3, 32], normal=0.005)  # 5x5 conv 64
        print kernel
        biases = bias_variable('biases', [32])
    ...
    with tf.variable_scope('conv2') as scope:
        kernel = weight_variable('weights', [5, 5, 32, 32], normal=0.005)  # 5x5 conv 64
        print kernel
        biases = bias_variable('biases', [32])
   ...
   #     max_pool_2 = tf.nn.avg_pool(conv2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='max_pool_2')
   #     # ksize[1,height,width,1]    |   strides[1,height,width,1]      maybe 2x2 is better
   #     norm_2 = tf.nn.lrn(max_pool_2, depth_radius=4, bias=1.0, alpha=0.001/9.0, beta=0.75, name='norm_2')
   #     # local response normalization all parameter from the formula

    with tf.variable_scope('conv3') as scope:
        kernel = weight_variable('weights', [5, 5, 32, 64], normal=0.005)
        biases = bias_variable('biases', [64])
        wa = tf.nn.conv2d(conv2, kernel, strides=[1, 1, 1, 1], padding='SAME')
        z = wa + biases
        conv3 = tf.nn.relu(z, name=scope.name)
        print conv3
        tf.summary.histogram(scope.name + '/activations', conv3)

    with tf.variable_scope('pool_norm_3') as scope:
        norm_3 = tf.nn.lrn(conv3, depth_radius=4, bias=1.0, alpha=0.001/9.0, beta=0.75, name='norm_3')
        # local response normalization all parameter from the formula
        max_pool_3 = tf.nn.avg_pool(norm_3, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='max_pool_3')
        # ksize[1,height,width,1]    |   strides[1,height,width,1]      maybe 2x2 is better

    # 再经过两个全连接层，映射到 64*192 的二维张量,

    with tf.variable_scope('full_connected_1') as scope:
        reshape = tf.reshape(max_pool_3, [BATCH_SIZE, -1])
