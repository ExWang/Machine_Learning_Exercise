...
def input_mnist_batch(data_set, batch_size, num_epochs, isShuffle):
    if not num_epochs:
        num_epochs = None
    if data_set == 'train':
        isTest = False
    ...
    min_queue_examples = int(examples_per_epoch*min_fraction_of_examples_in_queue)

    images, labels = _generate_image_and_label_batch(image, label, min_queue_examples=min_queue_examples,
                                                     batch_size=batch_size, shuffle=isShuffle)
